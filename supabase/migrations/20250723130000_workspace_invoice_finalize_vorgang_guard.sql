-- CLOUD-ORDER-CHAIN-03B1: Harden finalize_workspace_invoice
-- Require vorgang to exist in workspace_vorgaenge for the same workspace.
-- Does NOT alter 03A migration file. No new tables.
...
begin
  ...
  -- CLOUD-ORDER-CHAIN-03B1: reject orphan / foreign vorgang ids.
  select exists (
    select 1
  ...
  into v_vorgang_exists;

  if not coalesce(v_vorgang_exists, false) then
