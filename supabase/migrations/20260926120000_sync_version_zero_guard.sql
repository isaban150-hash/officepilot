-- SYNC-DURABILITY-HARDENING-01G -- Version 0 ist kein Freibrief.
--
-- Befund aus dem Audit 01F: Der generische Sync-RPC prueft die Version nur,
-- wenn der Client eine mitschickt (`p_row_version > 0`). Ein Client ohne
-- bestaetigte Version -- frisch angelegt, nach Neustart, nach verlorener
-- Bestaetigung -- konnte damit eine **neuere** Serverfassung ueberschreiben
-- und sogar einen Grabstein wiederbeleben. Genau das ist stiller Datenverlust.
--
-- Diese Migration aendert die Bedeutung nicht, sondern schaerft sie:
--
--   Version 0 heisst 'ich habe keine bestaetigte Serverversion'.
--
--   * Keine Zeile vorhanden -> CREATE wie bisher.
--   * Zeile vorhanden, Inhalt identisch -> Wiederholung eines Schreibvorgangs,
--     dessen Bestaetigung verloren ging: Die Zeile wird unveraendert
--     zurueckgegeben (`replayed`), ohne Versionssprung.
--   * Zeile vorhanden, Inhalt abweichend oder Grabstein -> Versionskonflikt.
--
-- Betroffen sind die vier Typen, deren Clients tatsaechlich mit Version 0
-- schreiben: vorgang, customer, vorgang_note, task. Die Singleton-Dokumente
-- (workspace, workspace_settings, company_setup, company_profile) bleiben
-- unveraendert -- dort ist Version 0 der regulaere Erstschreibweg eines
-- Arbeitsbereichs, und ihre Zweige haben eigene Bewahrungsregeln.
--
-- 01G2 -- geprueft und nachgezogen: Auch eine **negative** Version ist keine
-- bestaetigte Version. Vorher fiel sie durch beide Pruefungen (`= 0` und `> 0`)
-- und durfte damit ueberschreiben; der Guard greift jetzt bei `<= 0`.
--
-- 01G4 -- dieselbe Luecke ein drittes Mal, diesmal durch `NULL`. In SQL ist
-- jeder Vergleich mit `NULL` selbst `NULL` und damit nicht wahr: Ein Aufruf mit
-- `p_row_version = NULL` fiel deshalb durch **beide** Pruefungen (`<= 0` und
-- `<> v_current_version`) und durfte eine neuere Fassung ueberschreiben sowie
-- einen Grabstein wiederbeleben -- genau der stille Datenverlust, den diese
-- Migration verhindern soll. Nachgewiesen gegen die lokal angewandte Funktion
-- fuer vorgang, customer, vorgang_note und task.
--
-- Die Version kennt ab hier nur noch zwei Zustaende: bestaetigt (> 0) und
-- unbestaetigt. `NULL` ist keine bestaetigte Serverversion und wird deshalb
-- gleich zu Beginn als `0` gelesen -- ein dritter, ungepruefter Zustand kann
-- nicht mehr entstehen. Abweisen waere die Alternative gewesen; sie haette
-- einen solchen Aufruf dauerhaft blockiert, statt ihn in den bereits
-- gehaerteten Vertrag zu zwingen (Anlegen / Wiederholung / Konflikt).
--
-- Rein additiv: Die bereits remote angewandten Migrationen bleiben unberuehrt;
-- Basis ist die zuletzt wirksame Definition aus
-- 20260925120000_workspace_invoice_dunning_documentations.sql.
