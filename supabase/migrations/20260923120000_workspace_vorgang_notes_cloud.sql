-- CLOUD-DURABILITY-CORE-01B -- Vorgangsnotizen cloud-dauerhaft.
--
-- Notizen zu einem Vorgang lebten bisher ausschliesslich lokal
-- (`vorgang_note` stand in LOCAL_ONLY_SYNC_ENTITY_TYPES). Ein Geraetewechsel
-- oder ein geleerter Browserspeicher liess sie ersatzlos verschwinden.
--
-- Diese Migration ist **rein additiv**: eine neue Tabelle und je eine
-- zusaetzliche Verzweigung bzw. ein zusaetzlicher Schluessel in den beiden
-- bestehenden Sync-RPCs. Kein bestehender Zweig, Schluessel oder
-- Sicherheitscheck wird veraendert; Clients ohne den Entity-Typ
-- `vorgang_note` arbeiten unveraendert weiter.
--
-- Basis der beiden `create or replace function`: die jeweils zuletzt wirksame
-- Definition --
--   upsert_workspace_sync_entity  aus 20260919120000_company_profile_schema_preserve.sql
--   pull_workspace_sync_state     aus 20250829120000_workspace_customers_cloud.sql

create table if not exists public.workspace_vorgang_notes (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces (id) on delete cascade,
  client_note_id text not null,
  -- Bleibt erhalten: der Vorgangsbezug ist die einzige Ordnung der Notiz und
  -- muss auch am Grabstein lesbar bleiben.
  ...
  unique (workspace_id, client_note_id)
);

create index if not exists workspace_vorgang_notes_workspace_idx
  on public.workspace_vorgang_notes (workspace_id);

create index if not exists workspace_vorgang_notes_vorgang_idx
  on public.workspace_vorgang_notes (workspace_id, client_vorgang_id)
  where deleted = false;

-- Bestehende Trigger-Funktion aus 20250711140000 -- unveraendert wiederverwendet.
drop trigger if exists workspace_vorgang_notes_set_updated_at on public.workspace_vorgang_notes;
create trigger workspace_vorgang_notes_set_updated_at
before update on public.workspace_vorgang_notes
for each row
execute function public.set_workspace_updated_at();

-- Pull um Vorgangsnotizen erweitern
...
-- Upsert um den Entity-Typ vorgang_note erweitern
...
alter table public.workspace_vorgang_notes enable row level security;

/*
 * SELECT: jedes aktive Mitglied. Lokal zeigt die Vorgangsakte jedem Mitglied
 * alle Notizen des Vorgangs; die Cloud bildet genau das ab und verengt nicht.
 */
drop policy if exists workspace_vorgang_notes_select_member on public.workspace_vorgang_notes;
create policy workspace_vorgang_notes_select_member
on public.workspace_vorgang_notes for select to authenticated
...
revoke all on public.workspace_vorgang_notes from public, anon;
grant select on public.workspace_vorgang_notes to authenticated;
