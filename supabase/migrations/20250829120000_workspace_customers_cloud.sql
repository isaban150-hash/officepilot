-- PRODUCT-FOUNDATION-03A-S1: Kundenstamm cloud-syncable (nur Serverseite).
--
-- Der Kundenstamm lebte bisher ausschliesslich lokal. Rechnungen und Vorgaenge
-- bleiben auf einem zweiten Geraet zwar inhaltlich korrekt, weil sie ihre
-- Empfaengerdaten als Snapshot mitfuehren -- der Kundenstamm selbst und die
-- spaetere Rueckverknuepfung fehlen dort aber vollstaendig.
--
-- Diese Migration ist **rein additiv**: eine neue Tabelle und je eine
-- zusaetzliche Verzweigung in den beiden bestehenden Sync-RPCs. Kein
-- bestehender Zweig, Schluessel oder Sicherheitscheck wird veraendert.
-- Bestehende Clients kennen weder den Entity-Typ `customer` noch den neuen
-- Pull-Schluessel `customers` und arbeiten unveraendert weiter.
--
-- Der Client folgt erst in 03A-C1. `Vorgang.customerId` bleibt vollstaendig
-- unangetastet und ist erst Gegenstand von 03B.
--
-- Basis der beiden `create or replace function`: die letzte wirksame
-- Definition aus 20250711150000_workspace_vorgaenge_cloud.sql. Spaetere
-- Migrationen haben diese beiden Funktionen nicht veraendert.
...
/*
 * Bewusst KEIN Unique-Index auf Firmenname, E-Mail, Telefon oder Anschrift.
 * Zwei gleichnamige Kunden mit verschiedenen `customer_id` sind zwei Kunden --
 * kein Feld des Kundendatensatzes kann Firmenidentitaet beweisen, es gibt
 * weder USt-IdNr. noch Steuernummer. Eine falsche Zusammenfuehrung waere
 * praktisch nicht rueckgaengig zu machen; eine sichtbare Dublette schon.
 */
create index if not exists workspace_customers_workspace_id_idx
  on public.workspace_customers (workspace_id);

create index if not exists workspace_customers_workspace_active_idx
  on public.workspace_customers (workspace_id)
  where deleted = false;

-- Bestehende Trigger-Funktion aus 20250711140000 -- unveraendert wiederverwendet.
drop trigger if exists workspace_customers_set_updated_at on public.workspace_customers;
create trigger workspace_customers_set_updated_at
before update on public.workspace_customers
for each row
execute function public.set_workspace_updated_at();

-- Extend pull to include customers
...
-- Extend upsert for customer entity type
...
-- uebrigen Workspace-Tabellen.
revoke all on public.workspace_customers from public, anon;
