import type { ClassifiedDocumentKind } from './models';
import type { DocumentZone } from './documentAnalysis';
import type { DocumentFeatureStrength } from './documentFeatures';

export type CandidateFeatureRule = {
  featureId: string;
  weight: number;
  required?: boolean;
  zones?: DocumentZone[];
  minStrength?: DocumentFeatureStrength;
};

export type ReceiptCandidateProfile = {
  kind: ClassifiedDocumentKind;
  family: string;
  positive: CandidateFeatureRule[];
  negative: CandidateFeatureRule[];
  structural: CandidateFeatureRule[];
};

export const RECEIPT_CANDIDATE_PROFILES: ReceiptCandidateProfile[] = [
  {
    kind: 'tankbeleg',
    family: 'eingangsrechnung',
    /**
     * OFFICEPILOT-RECEIPT-PRIMARY-DOCUMENT-TYPE-FIX-01 — `structure.receipt_layout`
     * verlangt <= 12 bedeutsame Zeilen und misst damit Kompaktheit, keine
     * fachliche Dokumentidentität. Als einziges Profil führte `tankbeleg` es als
     * Pflichtmerkmal; ein langer, realistischer Beleg mit Terminalblock verlor
     * dadurch zusätzlich die Gesamtstrafe für fehlende Pflichtmerkmale, während
     * die Zahlungsprofile straffrei blieben. Die Gewichtung bleibt, die
     * Pflichtidentität tragen weiterhin `fuel_marker` und `monetary_value`.
     */
    structural: [
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body', 'header'] },
    ],
    positive: [
      {
        featureId: 'structure.fuel_marker',
        weight: 4,
        zones: ['body', 'header'],
        minStrength: 'medium',
        required: true,
      },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body', 'header'], required: true },
      { featureId: 'payment.card_payment', weight: 1.5, zones: ['body', 'header'] },
      { featureId: 'date.document_date', weight: 0.8, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 2, zones: ['body'] },
      { featureId: 'payment.iban', weight: 1.5, zones: ['body', 'footer'] },
      { featureId: 'structure.authority_letter', weight: 2, zones: ['header', 'body'] },
    ],
  },
  {
    kind: 'ec_beleg',
    family: 'eingangsrechnung',
    structural: [
      { featureId: 'structure.receipt_layout', weight: 2.5, zones: ['body'] },
    ],
    positive: [
      {
        featureId: 'structure.ec_marker',
        weight: 4,
        zones: ['body', 'header'],
        required: true,
      },
      { featureId: 'payment.card_payment', weight: 3, zones: ['body', 'header'], minStrength: 'medium' },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body'], required: true },
      { featureId: 'date.document_date', weight: 0.8, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.kreditkarten_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.fuel_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 2, zones: ['body'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'register.hrb_hra_number', weight: 1, zones: ['footer'] },
    ],
  },
  {
    kind: 'kreditkartenbeleg',
    family: 'eingangsrechnung',
    structural: [
      { featureId: 'structure.receipt_layout', weight: 2, zones: ['body'] },
    ],
    positive: [
      {
        featureId: 'structure.kreditkarten_marker',
        weight: 4,
        zones: ['body', 'header'],
        required: true,
      },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body'], required: true },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body', 'header'] },
      { featureId: 'date.document_date', weight: 0.8, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.ec_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.fuel_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.quittung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'register.hrb_hra_number', weight: 1, zones: ['footer'] },
    ],
  },
  {
    kind: 'kassenbeleg',
    family: 'eingangsrechnung',
    structural: [
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
    ],
    positive: [
      { featureId: 'amount.monetary_value', weight: 3, zones: ['body'] },
      { featureId: 'date.document_date', weight: 0.8, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'payment.card_payment', weight: 1, zones: ['body'] },
      { featureId: 'structure.fuel_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 2, zones: ['body', 'header'] },
      { featureId: 'register.hrb_hra_number', weight: 1, zones: ['footer'] },
    ],
  },
  {
    kind: 'quittung',
    family: 'eingangsrechnung',
    structural: [
      { featureId: 'structure.receipt_layout', weight: 2, zones: ['body'] },
    ],
    positive: [
      {
        featureId: 'structure.quittung_marker',
        weight: 4,
        zones: ['body', 'header'],
        required: true,
      },
      { featureId: 'amount.labeled_total', weight: 2.5, zones: ['body', 'header', 'footer'], required: true },
      { featureId: 'amount.monetary_value', weight: 2, zones: ['body'] },
      { featureId: 'date.document_date', weight: 0.8, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'payment.card_payment', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.kreditkarten_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.ec_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.fuel_marker', weight: 1.5, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 3, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'register.hrb_hra_number', weight: 1, zones: ['footer'] },
    ],
  },
  {
    kind: 'handelsregister',
    family: 'behoerde',
    structural: [],
    positive: [
      { featureId: 'register.hrb_hra_number', weight: 3, zones: ['footer', 'body', 'header'] },
      { featureId: 'register.court_marker', weight: 2.5, zones: ['footer', 'body', 'header'] },
      { featureId: 'register.managing_director_marker', weight: 2, zones: ['footer', 'body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.receipt_layout', weight: 4, zones: ['body'] },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body'] },
      { featureId: 'structure.fuel_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'finanzamt',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.finanzamt_marker',
        weight: 4,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.krankenkasse_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.soka_bau_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.agentur_fuer_arbeit_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'bg_bau',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.bg_bau_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
      { featureId: 'amount.labeled_total', weight: 1.5, zones: ['body'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.unbedenklichkeit_marker', weight: 4.5, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.finanzamt_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'structure.krankenkasse_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.soka_bau_marker', weight: 5, zones: ['body', 'header'] },
      { featureId: 'structure.agentur_fuer_arbeit_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'steuerbescheid',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.steuerbescheid_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.bg_bau_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.krankenkasse_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.soka_bau_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.agentur_fuer_arbeit_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'krankenkasse',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.krankenkasse_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.finanzamt_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.soka_bau_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.freistellung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.agentur_fuer_arbeit_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'soka_bau',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.soka_bau_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
      { featureId: 'amount.labeled_total', weight: 1.5, zones: ['body'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.finanzamt_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 5, zones: ['body', 'header'] },
      { featureId: 'structure.krankenkasse_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'structure.freistellung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.agentur_fuer_arbeit_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'agentur_fuer_arbeit',
    family: 'behoerde',
    structural: [
      {
        featureId: 'structure.authority_letter',
        weight: 3,
        zones: ['header', 'body'],
        required: true,
      },
    ],
    positive: [
      {
        featureId: 'structure.agentur_fuer_arbeit_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.deadline_date', weight: 1.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
      { featureId: 'structure.finanzamt_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.krankenkasse_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.soka_bau_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'freistellungsbescheinigung',
    family: 'behoerde',
    structural: [],
    positive: [
      {
        featureId: 'structure.freistellung_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'date.valid_until', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.document_date', weight: 1, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'structure.unbedenklichkeit_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
    ],
  },
  {
    kind: 'unbedenklichkeitsbescheinigung',
    family: 'behoerde',
    structural: [],
    positive: [
      {
        featureId: 'structure.unbedenklichkeit_marker',
        weight: 5,
        zones: ['header', 'body'],
        required: true,
      },
      { featureId: 'structure.bg_bau_marker', weight: 2.5, zones: ['header', 'body'] },
      { featureId: 'date.valid_until', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.document_date', weight: 1, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'structure.freistellung_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
    ],
  },
  {
    kind: 'eingangsrechnung',
    family: 'eingangsrechnung',
    structural: [],
    positive: [
      {
        featureId: 'reference.invoice_number',
        weight: 3.5,
        zones: ['body', 'header'],
        minStrength: 'medium',
        required: true,
      },
      { featureId: 'amount.labeled_total', weight: 2.5, zones: ['body'] },
      { featureId: 'amount.monetary_value', weight: 1.5, zones: ['body'] },
      { featureId: 'payment.iban', weight: 1.5, zones: ['body', 'footer'] },
      { featureId: 'date.deadline_date', weight: 1.5, zones: ['body'] },
    ],
    negative: [
      { featureId: 'structure.payment_request', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 4, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'structure.fuel_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.card_payment', weight: 1.5, zones: ['body'] },
    ],
  },
  {
    kind: 'mahnung',
    family: 'zahlung',
    structural: [],
    positive: [
      {
        featureId: 'structure.mahnung_marker',
        weight: 4,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.payment_request',
        weight: 3.5,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      { featureId: 'structure.weak_payment_signal', weight: 1, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'reference.invoice_number', weight: 1.5, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'amount.labeled_total', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.zahlungserinnerung_marker', weight: 5, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
    ],
  },
  {
    kind: 'zahlungserinnerung',
    family: 'zahlung',
    structural: [],
    positive: [
      {
        featureId: 'structure.zahlungserinnerung_marker',
        weight: 6,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.payment_request',
        weight: 3.5,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      { featureId: 'structure.weak_payment_signal', weight: 1, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.deadline_date', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'amount.monetary_value', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'reference.invoice_number', weight: 1.5, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'amount.labeled_total', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'payment.card_payment', weight: 2, zones: ['body'] },
    ],
  },
  {
    kind: 'werkvertrag',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.werkvertrag_marker',
        weight: 6,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.contract_party_marker',
        weight: 3,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 2, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.contract_date', weight: 2, zones: ['header', 'body'] },
      { featureId: 'reference.case_reference', weight: 1.5, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'structure.subunternehmer_marker', weight: 4, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.nachunternehmer_marker', weight: 3.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.freistellung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.unbedenklichkeit_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.finanzamt_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 2, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'subunternehmervertrag',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.subunternehmer_marker',
        weight: 5,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.contract_party_marker',
        weight: 3,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 2, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.contract_date', weight: 2, zones: ['header', 'body'] },
      { featureId: 'reference.case_reference', weight: 1.5, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'structure.werkvertrag_marker', weight: 5, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.nachunternehmer_marker', weight: 3.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.freistellung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.unbedenklichkeit_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.finanzamt_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 2, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'nachunternehmervertrag',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.nachunternehmer_marker',
        weight: 5,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.contract_party_marker',
        weight: 3,
        zones: ['body', 'header', 'footer'],
        required: true,
      },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 2, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.contract_date', weight: 2, zones: ['header', 'body'] },
      { featureId: 'reference.case_reference', weight: 1.5, zones: ['body', 'header', 'footer'] },
    ],
    negative: [
      { featureId: 'structure.werkvertrag_marker', weight: 3, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.subunternehmer_marker', weight: 3.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'structure.freistellung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.unbedenklichkeit_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'structure.receipt_layout', weight: 3, zones: ['body'] },
      { featureId: 'reference.invoice_number', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2, zones: ['body', 'footer'] },
      { featureId: 'structure.finanzamt_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.steuerbescheid_marker', weight: 2.5, zones: ['body', 'header'] },
      { featureId: 'structure.bg_bau_marker', weight: 2, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'auftrag',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.auftrag_marker',
        weight: 5,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.customer_party_marker',
        weight: 2.5,
        zones: ['header', 'body', 'footer'],
      },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.document_date', weight: 1.5, zones: ['header', 'body'] },
      { featureId: 'amount.labeled_total', weight: 2, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.angebot_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.auftragsbestaetigung_marker', weight: 5, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.werkvertrag_marker', weight: 5, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.subunternehmer_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.nachunternehmer_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.contract_party_marker', weight: 3, zones: ['header', 'body', 'footer'] },
      { featureId: 'date.contract_date', weight: 3, zones: ['header', 'body'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 3, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'structure.finanzamt_marker', weight: 2, zones: ['body', 'header'] },
      { featureId: 'structure.freistellung_marker', weight: 2.5, zones: ['body', 'header'] },
    ],
  },
  {
    kind: 'angebot',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.angebot_marker',
        weight: 5,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.customer_party_marker',
        weight: 2.5,
        zones: ['header', 'body', 'footer'],
      },
      { featureId: 'reference.case_reference', weight: 2, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.document_date', weight: 1.5, zones: ['header', 'body'] },
      { featureId: 'amount.labeled_total', weight: 2.5, zones: ['body', 'header'] },
    ],
    negative: [
      { featureId: 'structure.auftrag_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.auftragsbestaetigung_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.werkvertrag_marker', weight: 5, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.contract_party_marker', weight: 3, zones: ['header', 'body', 'footer'] },
      { featureId: 'date.contract_date', weight: 3, zones: ['header', 'body'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'structure.mahnung_marker', weight: 3, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2.5, zones: ['body', 'footer'] },
      { featureId: 'structure.nachunternehmer_marker', weight: 3, zones: ['header', 'body', 'footer'] },
    ],
  },
  {
    kind: 'auftragsbestaetigung',
    family: 'kunde',
    structural: [],
    positive: [
      {
        featureId: 'structure.auftragsbestaetigung_marker',
        weight: 5,
        zones: ['header', 'body', 'footer'],
        required: true,
      },
      {
        featureId: 'structure.customer_party_marker',
        weight: 2.5,
        zones: ['header', 'body', 'footer'],
      },
      { featureId: 'reference.case_reference', weight: 2.5, zones: ['body', 'header', 'footer'] },
      { featureId: 'date.document_date', weight: 1.5, zones: ['header', 'body'] },
    ],
    negative: [
      { featureId: 'structure.auftrag_marker', weight: 1, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.angebot_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.werkvertrag_marker', weight: 5, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.leistungsverzeichnis_marker', weight: 4, zones: ['header', 'body', 'footer'] },
      { featureId: 'structure.contract_party_marker', weight: 3, zones: ['header', 'body', 'footer'] },
      { featureId: 'date.contract_date', weight: 3, zones: ['header', 'body'] },
      { featureId: 'structure.payment_request', weight: 3.5, zones: ['body', 'header'] },
      { featureId: 'reference.invoice_number', weight: 3, zones: ['body', 'header'] },
      { featureId: 'payment.iban', weight: 2.5, zones: ['body', 'footer'] },
    ],
  },
  {
    kind: 'sonstiges',
    family: 'sonstiges',
    structural: [],
    positive: [],
    negative: [],
  },
];
