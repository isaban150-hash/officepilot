...
export type PaymentCutoverDetection = {
  kind: ClassifiedDocumentKind;
  reasonKey: string;
};

export type PaymentCutoverDecision = {
  eligible: boolean;
  detection?: PaymentCutoverDetection;
...
export function computePaymentCutoverConfidence(pipeline: ReceiptAnalysisPipelineResult): number {
...
export function evaluatePaymentCutoverEligibility(
  pipeline: ReceiptAnalysisPipelineResult | null,
): PaymentCutoverDecision {
  ...
  const cutoverConfidence = computePaymentCutoverConfidence(pipeline);
  const kindThresholds = getPaymentCutoverKindThresholds(winnerKind);

  if (!isPaymentScoringCutoverKind(winnerKind) || !kindThresholds) {
    return { eligible: false, rejectionReason: 'cutover:winner_not_allowed' };
  }

  if (!hasPaymentCutoverKindTextGuard(winnerKind, pipeline.recognizedText)) {
  ...
  if (ocrQuality.score < PAYMENT_SCORING_CUTOVER.minOcrScore) {
  ...
  if (evidenceCount < PAYMENT_SCORING_CUTOVER.minEvidenceRefs) {
  ...
  return {
    ...
    detection: {
      kind: winnerKind,
      reasonKey: DI_PAYMENT_SCORING_REASON_KEY,
    },
  };
}
