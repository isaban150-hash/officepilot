...
/**
 * BUSINESS-STATE-DIRECT-CONFIRMATION-01E — which party IS the own company.
 *
 * Identity first, role second. A pure role rule would be unsafe: in a
 * subcontract the user can just as well be the Auftraggeber, and then the
 * foreign company sits in the Auftragnehmer slot. So the own party is found by
 * who it is, and its role only confirms the direction afterwards.
 *
 ...
 */
function isOwnCompanyParty(party: DetectedContractParty, profile: CompanyProfile): boolean {
