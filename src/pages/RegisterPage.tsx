...
export function RegisterPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (!acceptTerms || !acceptPrivacy || !acceptLicense) {
      setError(getAuthErrorMessage('terms_required'));
      return;
    }
    setLoading(true);
  ...
  return (
    ...
    >
      <form className="auth-form" onSubmit={handleSubmit}>
        ...
        <label className="auth-form__field">
          <span>Passwort</span>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} data-testid="register-password" />
        </label>
        <label className="auth-form__field">
        ...
        <label className="auth-form__checkbox">
          <input type="checkbox" checked={acceptTerms} onChange={(e) => setAcceptTerms(e.target.checked)} required data-testid="register-terms" />
          <span>
            Ich akzeptiere die{' '}
        ...
        <label className="auth-form__checkbox">
          <input type="checkbox" checked={acceptPrivacy} onChange={(e) => setAcceptPrivacy(e.target.checked)} required data-testid="register-privacy" />
          <span>
            Ich habe die{' '}
        ...
        <label className="auth-form__checkbox">
          <input type="checkbox" checked={acceptLicense} onChange={(e) => setAcceptLicense(e.target.checked)} required data-testid="register-license" />
          <span>
            Ich akzeptiere die{' '}
